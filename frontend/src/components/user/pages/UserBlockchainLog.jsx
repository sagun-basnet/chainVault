...
const UserBlockchainLog = () => {
  ...
  const fetchLogs = async () => {
    try {
      const res = await axios.get(`http://localhost:5550/api/files/blockchain/logs?userId=${userId}`);
      setLogs(res.data);
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white mb-2">My Blockchain Ledger</h1>
          <p className="text-gray-400">Immutable record of your file activities on the blockchain</p>
        </div>
...
export default UserBlockchainLog;
